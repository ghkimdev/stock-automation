...
- **기술적 지표 계산**: 이동평균(MA5/20/60), RSI, MACD, 볼린저밴드, 스토캐스틱, ATR, ADX
- **매매 신호 생성**: 5개 지표 다수결 BUY/SELL 신호 + ADX/거래량 필터로 노이즈 제거
- **손절/목표가 자동 계산**: ATR 기반 (기본 RR 2:1)
...
==================================================
  종목: AAPL
==================================================
  📈 [2024-03-15] BUY  | 진입:     172,000 | 손절:     169,400 | 목표:     177,200 | RSI 과매도(28.3), BB 하단 터치
  📉 [2024-07-22] SELL | 진입:     195,000 | 손절:     197,900 | 목표:     186,300 | MA 데드크로스, RSI 과매수(74.1)
```
...
| 스토캐스틱 | 과매도 영역(%K<%D→%K>%D) | 과매수 영역(%K>%D→%K<%D) |
...
| 필터 | 조건 | 효과 |
|------|------|------|
| ADX | ADX < 20 이면 신호 무효 | 횡보장 오신호 차단 |
| 거래량 | 거래량 < 20일 평균이면 신호 무효 | 거래 없는 신호 차단 |
...
| 손절가 (BUY) | 진입가 − ATR × 1.5 |
| 목표가 (BUY) | 진입가 + ATR × 3.0 |
| 손절가 (SELL) | 진입가 + ATR × 1.5 |
| 목표가 (SELL) | 진입가 − ATR × 3.0 |
...
├── indicators/
│   ├── moving_average.py  # 이동평균 + 골든/데드크로스
│   ├── rsi.py             # RSI
│   ├── macd.py            # MACD
│   ├── bollinger.py       # 볼린저밴드
│   ├── stochastic.py      # 스토캐스틱
